		...
		<div class="content">
			<header>
			 ...
			 <div style="display: flex;justify-content: center;">
			      <el-avatar icon="el-icon-user-solid" :size="90" style="font-size: 65px;margin-top: 10px;background: rgba(255,255,255,0.1);"></el-avatar>
			</div>
			
			<div class="demo-input-suffix">
			  <el-input
			    placeholder="请输入用户名"
			    v-model="input1" width="100%" autofocus>
			  </el-input>
			  <el-input
			    placeholder="请输入密码"
			    v-model="input2" width="100%" type="password">
			  </el-input>
			  <p class="pBtn">
			...
			logins(){
				...
				this.timer=setTimeout(()=>{//防抖操作
					...
					if(this.input1.trim()==''|| this.input2.trim()==''){
						...
						var that = this;
						that.result = that.userList.some(item=>{
						   if(item.username===that.input1 && item.password === that.input2){
							   this.thisUserId = item.id;
						      return true 
						  } else{
							  return false
						  }
						...
						setTimeout(()=>{
							if(this.result){
								...
								window.localStorage.setItem('username',this.input1);
								window.localStorage.setItem('loginInfo',tokens);
								window.localStorage.setItem('loginThisId',this.thisUserId);//本地存储当前登录用户id
...
<style scoped="scoped">
	...
	.pBtn{
		display: flex;justify-content: space-between;padding: 20px 0px;
	}
	.pBtn span{
		cursor: pointer;
	}
